...
export const OpacityControl: React.FC<OpacityControlProps> = ({ label, value, onSave, className = '', variant = 'overlay' }) => {
    const [isEditing, setIsEditing] = useState(variant === 'sidebar');

    const safeValue = (typeof value === 'number' && !isNaN(value)) ? value : 100;

    return (
        <div className={`relative group/opacity ${className}`}>
            {isEditing ? (
                <div className={`${variant === 'sidebar' ? 'relative w-full mb-4' : 'absolute right-0 top-0 z-[1000] bg-zinc-900 border border-zinc-700 p-3 rounded-xl shadow-2xl flex flex-col gap-2 w-48'}`} onClick={e => e.stopPropagation()}>
                    <div className="flex justify-between items-center">
                        <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest">{label} Opacity</span>
                        {variant === 'overlay' && (
